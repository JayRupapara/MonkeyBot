...
import DailyCheckInPopup from "../components/DailyCheckInPopup"; // Import the new popup component

const Home = () => {
  const navigate = useNavigate();
  const { monkeyCoins, setMonkeyCoins } = useMonkey(); 
  const [showPopup, setShowPopup] = useState(false);
  const [showCheckInPopup, setShowCheckInPopup] = useState(false); // State for daily check-in popup
  ...
  const updateCoins = (newCoins) => {
    console.log("updateCoins in Home before passing to Popup:", updateCoins); 
  ...
  return (
    <div className="bg-black text-white flex flex-col min-h-screen">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col items-center p-4">
        ...
        <section className="my-4 w-full">
          <div className="flex items-center justify-between">
            ...
            <button
              className="bg-white text-black px-4 py-2 rounded-lg ml-4 shadow-lg font-bold"
              onClick={handleDailyCheckInClick}  // Open Daily Check-In popup
            >
              Daily Check-In
            </button>
          </div>
          ...
          {/* Invite Task with handleInviteClick */}
          <div className="bg-gray-800 p-4 mt-4 rounded-lg flex justify-between items-center shadow-md border border-white">
            <div>
      ...
      {showCheckInPopup && (
        <DailyCheckInPopup onClose={() => setShowCheckInPopup(false)} />
      )}
    </div>
  );
};
